...
- **Unit of work is one block, transformed in place.** Encrypt loads the deeplinked block's existing content, encrypts it, and `updateBlock`s that same block into a single-line `HIDDEN-CAP:` code block — it does not create or replace with fresh content. Decrypt reads that block, decrypts, and renders in Quill without persisting. Rationale: the owner's model is "turn the existing block into encrypted form"; this keeps the object identity and deeplink stable across the transform.
...
- [The exact Capacities **code-block type string** and how a single-line code block is represented on the wire are not yet confirmed] → verify live against a real workspace the same way `TextBlock`/`TextToken` were discovered during the `unofficial_capacities` build, before finalizing the encrypt path.
