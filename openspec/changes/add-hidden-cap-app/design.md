...
- **Unit of work is one block, transformed in place, same type.** Encrypt loads the deeplinked block's existing content, encrypts it, and `updateBlock`s **that same block** with a same-type replacement carrying the `HIDDEN-CAP:` blob — a `TextBlock` stays a `TextBlock` (single token = the blob), a `CodeBlock` stays a `CodeBlock`. Decrypt reads that block, decrypts, and renders in Quill without persisting. Rationale: the owner's model is "turn the existing block into encrypted form"; an in-place update keeps the block's **position** in the object and its **id/deeplink stable**. The Capacities update API does not permit a block **type** change, so we never change type — we keep the original's. (An earlier build appended a new `CodeBlock` and deleted the original to force a code block; that dropped the encrypted block to the bottom of the object and changed its deeplink, so it was replaced by this in-place approach. Trade-off accepted: an encrypted plain paragraph is stored as text, not monospace code.)
